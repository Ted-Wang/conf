...
function changesrc(){
    ...
    if [[ "$os" == "ubuntu" ]]; then
        ...
        src_url_ustc='https://mirrors.sjtug.sjtu.edu.cn/'
...
echo        sudo sed -i "s|http://.*archive.ubuntu.com/|${src_url}|g" ${src_file_ubuntu}
echo        sudo sed -i "s|http://security.ubuntu.com/|${src_url}|g" ${src_file_ubuntu}
    fi
    if [[ "$os" == "raspbian" ]]; then
